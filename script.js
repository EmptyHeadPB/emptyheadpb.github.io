...
function initEventListeners() {
    ...
    document.getElementById('terms-btn')?.addEventListener('click', (e) => {
        e.preventDefault();
        showNotification('Условия использования', 'Сервис предоставляется бесплатно для некоммерческого использования.', 'info');
    });
    ...
    if (navigator.share) {
        try {
            ...
            await navigator.share({
                files: [file],
                title: 'Мой QR-код',
                text: 'Создано с помощью GlassQR Generator'
            });
